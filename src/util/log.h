...
public:
    ...
    static inline void assert(Args&&... args) {
...
#define ZIP_ASSERT(condition, message...) {                                                         \
    if (!(condition)) {                                                                             \
        zip::util::logger::assert("Assert failed at ", __FILE__, ":", __LINE__, " with ", message); \
        std::exit(1);                                                                               \
    }                                                                                               \
}                                                                                                   \
...
#define ZIP_ASSERT_EQ(expression, expected, message...) {                                                                           \
    ...
    if (value != expected) {                                                                                                        \
        zip::util::logger::assert("Assert failed at ", __FILE__, ":", __LINE__, " (", value, " != ", expected, ") with ", message); \
        std::exit(1);                                                                                                               \
    }                                                                                                                               \
}
